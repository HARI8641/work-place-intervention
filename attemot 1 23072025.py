...
# Style constants for interactive input boxes
FOCUS_BG = "#e6f0ff"
...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        label = tk.Label(tw, text=self.text, justify=tk.LEFT,
                         ...
                         font=("Segoe UI", "10", "normal"), wraplength=300)
...
# InteractiveEntry - Entry with focus and error border coloring
...
# Your original helper functions, unchanged, except color update on errors in submit_form
...
def admin_password_management_gui(parent=None):
    def on_change_user_pw():
        change_password("User", parent=admin_window)

    def on_change_admin_pw():
        change_password("Admin", parent=admin_window)

    admin_window = tk.Toplevel(parent)
    admin_window.title("Admin Password Management")
    admin_window.geometry("350x150")
    admin_window.resizable(False, False)

    tk.Label(admin_window, text="Admin Panel - Change Passwords", font=("Segoe UI", 14, "bold")).grid(row=0, column=0,
                                                                                                    columnspan=2, pady=10)
    btn_user_pw = tk.Button(admin_window, text="Change User Password", command=on_change_user_pw, width=25,
                            bg="#0078d7", fg="white", activebackground="#005a9e", relief="raised", cursor="hand2")
    btn_user_pw.grid(row=1, column=0, padx=20, pady=10)
    btn_admin_pw = tk.Button(admin_window, text="Change Admin Password", command=on_change_admin_pw, width=25,
                             bg="#0078d7", fg="white", activebackground="#005a9e", relief="raised", cursor="hand2")
    btn_admin_pw.grid(row=1, column=1, padx=20, pady=10)
    btn_close = tk.Button(admin_window, text="Close", command=admin_window.destroy, width=52,
                          bg="#555", fg="white", activebackground="#333", relief="raised", cursor="hand2")
    btn_close.grid(row=2, column=0, columnspan=2, pady=15)
...
def create_patient_id_card(info, qr_filename, output_filename):
    ...
    draw.text(((w - draw.textlength("Patient ID Card", title_font)) // 2, margin + 210), "Patient ID Card",
              font=title_font, fill="red")

    # Patient ID big and centered
    draw.text(((w - draw.textlength(info["id"], id_font)) // 2, margin + 270), info["id"], font=id_font, fill="blue")

    # Details start coordinates
    x_label = margin + 60
    x_colon = x_label + 300
    x_value = x_colon + 20
    y_start = margin + 400
...
def reset_form():
    ...
    if name_entry:
        ...
        name_entry.mark_error(False)
    if dob_entry:
        dob_entry.delete(0, tk.END)
        dob_entry.mark_error(False)
    if calendar_widget:
    ...
    if care_of_entry:
        ...
        care_of_entry.mark_error(False)
    if phone_entry:
        phone_entry.delete(0, tk.END)
        phone_entry.mark_error(False)
    if age_var:
        age_var.set("")
...
def submit_form():
    ...
    for ctl in [name_entry, dob_entry, care_of_entry, phone_entry]:
        ctl.mark_error(False)
    gender_combobox.config(background="white")
    ...
    error_found = False
    # Validation with error highlight & message
    if not name:
        name_entry.mark_error(True)
        error_found = True

    if not dob or not validate_date(dob):
        dob_entry.mark_error(True)
        error_found = True

    if not gender:
        gender_combobox.config(background=ERROR_BORDER_COLOR)
        error_found = True

    if not phone.isdigit() or len(phone) != 10:
        phone_entry.mark_error(True)
        error_found = True
...
def start_gui(root):
    ...
    app.geometry("1460x1020")
    app.configure(background="#f8f9fa")
    app.resizable(True, True)  # Let user resize if they want

    # Containers with some padding & background for visual grouping
    form_frame = tk.Frame(app, bg="#f8f9fa", padx=20, pady=20)
    form_frame.grid(row=0, column=0, sticky="nw")

    # Labels + interactive entry fields with tooltip

    tk.Label(form_frame, text="Patient Name:", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=0, column=0, sticky="w", pady=7)
    name_entry = InteractiveEntry(form_frame, width=32, font=("Segoe UI", 12))
    name_entry.grid(row=0, column=1, pady=7, padx=5)
    ToolTip(name_entry, "Enter full legal name of patient")

    tk.Label(form_frame, text="Date of Birth (dd-mm-yyyy):", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=1, column=0, sticky="w", pady=7)
    dob_entry = InteractiveEntry(form_frame, width=32, font=("Segoe UI", 12))
    dob_entry.grid(row=1, column=1, pady=7, padx=5)
    ToolTip(dob_entry, "Format: dd-mm-yyyy. Date cannot be future date.")

    tk.Label(form_frame, text="Click Calendar to Set DOB:", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=2, column=0, sticky="w", pady=7)
    calendar_widget = Calendar(form_frame, date_pattern="dd-mm-yyyy", mindate=datetime.datetime(1900, 1, 1),
                               maxdate=datetime.datetime.today(), background='white', foreground='black',
                               headersbackground='#0078d7', normalbackground='white', normalforeground='black',
                               weekendbackground='#e6f0ff', weekendforeground='black')
    calendar_widget.grid(row=2, column=1, pady=7)
    calendar_widget.selection_clear()

    tk.Label(form_frame, text="Age (in years):", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=3, column=0, sticky="w", pady=7)
    age_var = tk.StringVar()
    age_label = tk.Label(form_frame, textvariable=age_var, width=28, anchor="w", relief="sunken", font=("Segoe UI", 12),
                         bg="white", fg="#333")
    age_label.grid(row=3, column=1, pady=7, padx=5)

    tk.Label(form_frame, text="Gender:", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=4, column=0, sticky="w", pady=7)
    gender_combobox = Combobox(form_frame, values=["Male", "Female", "Other"], state="readonly", width=28,
                               font=("Segoe UI", 12))
    gender_combobox.grid(row=4, column=1, pady=7, padx=5)
    ToolTip(gender_combobox, "Select patient's gender")

    tk.Label(form_frame, text="Care Of:", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=5, column=0, sticky="w", pady=7)
    care_of_entry = InteractiveEntry(form_frame, width=32, font=("Segoe UI", 12))
    care_of_entry.grid(row=5, column=1, pady=7, padx=5)
    ToolTip(care_of_entry, "Name of guardian/caretaker if applicable")

    tk.Label(form_frame, text="Phone No:", anchor="w", width=20, font=("Segoe UI", 12, "bold"),
             bg="#f8f9fa", fg="#0078d7").grid(row=6, column=0, sticky="w", pady=7)
    phone_entry = InteractiveEntry(form_frame, width=32, font=("Segoe UI", 12))
    phone_entry.grid(row=6, column=1, pady=7, padx=5)
    ToolTip(phone_entry, "10-digit mobile number without country code")

    # Button with hover/active colors
    btn_generate = tk.Button(form_frame, text="Generate ID Card", width=32, command=submit_form,
                             bg="#0078d7", fg="white", activebackground="#005a9e", relief="raised", cursor="hand2",
                             font=("Segoe UI", 12, "bold"))
    btn_generate.grid(row=7, column=0, columnspan=2, pady=20, padx=5)
    ...
    btn_preview = tk.Button(form_frame, text="Preview Last ID Card", width=32, command=preview_last_id,
                            bg="#555", fg="white", activebackground="#333", relief="raised", cursor="hand2",
                            font=("Segoe UI", 11))
    btn_preview.grid(row=8, column=0, columnspan=2, pady=5, padx=5)

    # Preview toggle
    preview_enabled = tk.BooleanVar(value=True)
    btn_toggle = tk.Checkbutton(form_frame, text="Show Live ID Preview", variable=preview_enabled, bg="#f8f9fa",
                                font=("Segoe UI", 11))
    btn_toggle.grid(row=9, column=0, columnspan=2, pady=15, padx=5)

    # Preview frame with border & shadow look
    preview_frame = tk.Frame(app, relief="groove", bd=3, bg="white")
    preview_frame.grid(row=0, column=1, padx=15, pady=15, sticky="ne")
    preview_label = tk.Label(preview_frame, text="Live ID Card Preview", font=("Segoe UI", 14, "bold"), fg="#b22222",
                             bg="white")
    preview_label.pack(pady=5)

    preview_canvas = tk.Label(preview_frame, bg="white", width=930, height=1290, relief="ridge", bd=2)
    preview_canvas.pack()

    def create_temp_id_card(temp_path):
        # For preview use dummy ID + data with placeholders
        info = {
            "id": "PREVIEW-ID",
    ...
    def update_preview(event=None):
        ...
        try:
            ...
            img = img.resize((930, 1290), Image.LANCZOS)
            img_tk = ImageTk.PhotoImage(img)
            preview_canvas.config(image=img_tk, text="", bg="white")
            preview_canvas.image = img_tk  # keep ref
        except Exception as e:
    ...
    # Bind fields for live preview & sync DOB/calendar
    ...
    dob_entry.bind("<FocusOut>", sync_dob_field_to_calendar)
    dob_entry.bind("<KeyRelease>", sync_dob_field_to_calendar)
    calendar_widget.bind("<<CalendarSelected>>", sync_calendar_to_dob_field)
    btn_toggle.config(command=update_preview)

    sync_dob_field_to_calendar()
    update_preview()

    # Grid weight setup for resizing
    app.grid_columnconfigure(0, weight=1, minsize=470)
    app.grid_columnconfigure(1, weight=1, minsize=965)
